    ...
    public function updated(Event $event)
    {
        if ($event->events()->exists() || $event->event) {
            ...
            foreach($childEvents as $childEvent) {
                if($startTime) {
                    $childEvent->start = Carbon::parse($childEvent->start_time)->addSeconds($startTime)->toDateTimeString();
                }

                if($endTime) {
                    $childEvent->end = Carbon::parse($childEvent->end_time)->addSeconds($endTime)->toDateTimeString();
                }
